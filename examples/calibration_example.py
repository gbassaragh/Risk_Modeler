...
def generate_sample_cost_data():
    ...
    seasonal_factor = 1 + 0.1 * np.sin(2 * np.pi * np.arange(n_samples) / 52)  # Annual cycle
...
def demonstrate_seasonality_analysis():
    ...
    if seasonality_result.has_seasonality:
        ...
        if seasonality_result.seasonal_factors is not None:
            for i, factor in enumerate(seasonality_result.seasonal_factors[:12]):  # Show first 12
...
def create_visualization():
    ...
    try:
        ...
        # Histogram of data
        axes[0, 0].hist(costs, bins=30, density=True, alpha=0.7, color='lightblue', edgecolor='black')
        ...
        for i, (result, color) in enumerate(zip(results[:3], colors)):
            ...
            try:
                ...
                if dist_name == 'normal':
                    y = stats.norm.pdf(x, loc=result.parameters['loc'], scale=result.parameters['scale'])
                elif dist_name == 'lognormal':
                    y = stats.lognorm.pdf(x, s=result.parameters['s'], scale=result.parameters['scale'])
                elif dist_name == 'gamma':
                    y = stats.gamma.pdf(x, a=result.parameters['a'], scale=result.parameters['scale'])
                
                axes[0, 1].plot(x, y, color=color, label=f"{dist_name} (AIC: {result.aic:.1f})", linewidth=2)
            except:
                pass
