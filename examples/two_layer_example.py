...
def simple_cost_model(epistemic_params, n_samples, random_state, **kwargs):
    ...
    material_costs = DistributionSampler.sample(material_cost_dist, n_samples, random_state)
...
def main():
    ...
    two_layer = TwoLayerMonteCarlo(
        config=config,
        epistemic_parameters=epistemic_params,
        random_seed=42
    )
    ...
    print("\nInterpretation:")
    print("-" * 15)
    print("The confidence bands show the range of percentiles due to parameter uncertainty.")
    ...
    for percentile in config.target_percentiles:
        ...
        print(f"P{int(percentile):2d}: ${lower:8,.0f} - ${mean:8,.0f} - ${upper:8,.0f}  "
...
def advanced_example():
    ...
    def project_model(epistemic_params, n_samples, random_state):
        ...
        # Sample costs
        foundation_costs = DistributionSampler.sample(foundation_dist, n_samples, random_state)
        equipment_costs = DistributionSampler.sample(equipment_dist, n_samples, random_state)
