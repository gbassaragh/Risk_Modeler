...
class ResultsCalculator:
    ...
    def calculate_percentiles(
        data: np.ndarray, percentiles: List[float] = [10, 50, 80, 90, 95]
    ) -> Dict[str, float]:
...
class ChartGenerator:
    ...
    def create_tornado_diagram(
        self, tornado_data: Dict[str, Any], title: str = "Sensitivity Analysis"
    ) -> Any:
        ...
        if self.style == "plotly":
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    orientation="h",
                    marker_color=[
                        self.colors[0] if s >= 0 else self.colors[3]
            ...
            colors = [
                self.colors[0] if s >= 0 else self.colors[3] for s in sensitivities
            ]
    ...
    def create_pareto_chart(
        ...
        if self.style == "plotly":
            ...
            fig.add_trace(
                go.Bar(
                    x=labels, y=values, name=value_label, marker_color=self.colors[0]
                ),
                secondary_y=False,
            )
    ...
    def create_wbs_breakdown_chart(
        self,
        wbs_data: Dict[str, Dict[str, float]],
        metric: str = "mean",
        title: str = "WBS Cost Breakdown",
    ) -> Any:
        ...
        if self.style == "plotly":
            fig = go.Figure(data=[go.Pie(labels=wbs_codes, values=values, hole=0.3)])
...
class ReportGenerator:
    ...
    def create_charts_package(self, output_dir: str) -> List[str]:
        ...
        try:
            ...
            if self.results.risk_contributions:
                risk_data = {
                    risk_id: data.get("variance_contribution", 0)
    ...
    def _generate_key_insights(self) -> List[str]:
        ...
        if "P10" in percentiles and "P90" in percentiles:
            ...
            insights.append(
                f"Cost uncertainty range: {range_pct:.0f}% of base cost (P10-P90)"
            )
        ...
        if "P50" in percentiles:
            ...
            insights.append(
                f"Expected cost increase: {expected_increase:.1f}% above base case"
            )
        ...
        if "P80" in percentiles:
            ...
            insights.append(
                f"Recommended contingency (P80): {contingency_pct:.1f}% of base cost"
            )

        # Risk insights
        if self.results.risk_contributions:
            top_risk = max(
                self.results.risk_contributions.items(),
            ...
            insights.append(
                f"Top risk driver: {risk_id} ({contribution:.1f}% of variance)"
            )
