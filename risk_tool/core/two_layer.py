...
class TwoLayerResults:
    ...
    def summary_table(self) -> str:
        ...
        lines.append(
            f"Aleatory samples per epistemic: {len(self.aleatory_samples[0]) if self.aleatory_samples else 0}"
        )
        ...
        for p in self.target_percentiles:
            ...
            if key in self.percentile_bands:
                band = self.percentile_bands[key]
                lines.append(
                    f"{key:>3}: {band['lower']:>10.2f} | {band['mean']:>10.2f} | {band['upper']:>10.2f}"
                )
...
class EpistemicParameter:
    ...
    def sample(self, n_samples: int, random_state: np.random.RandomState) -> np.ndarray:
        ...
        return DistributionSampler.sample(
            self.distribution_config, n_samples, random_state
        )
...
class TwoLayerMonteCarlo:
    ...
    def _validate_config(self):
        ...
        if not (0 < lower < upper < 100):
            raise ValueError(
                f"Confidence band must be (lower, upper) with 0 < lower < upper < 100"
            )

    def run_simulation(
        self, aleatory_model: Callable, model_args: Dict[str, Any] = None
    ) -> TwoLayerResults:
        ...
        if self.config.parallel_processing:
            results = self._run_parallel_simulation(
                epistemic_samples, aleatory_model, model_args
            )
        else:
    ...
    def _run_sequential_simulation(
        ...
        for i, ep_params in enumerate(epistemic_samples):
            ...
            aleatory_result = aleatory_model(
                ep_params, self.config.n_aleatory, al_random_state, **model_args
            )
    ...
    def _run_parallel_simulation(
        ...
        max_workers = self.config.max_workers or min(
            multiprocessing.cpu_count(), n_epistemic
        )
    ...
    def _process_results(
        ...
        epistemic_means = np.mean(
            percentile_results, axis=1
        )  # Mean across percentiles for each epistemic
        epistemic_stats = {
            "mean": np.mean(epistemic_means),
            "std": np.std(epistemic_means),
            "cv": (
                np.std(epistemic_means) / np.mean(epistemic_means)
                if np.mean(epistemic_means) != 0
                else 0
            ),
        }
        ...
        aleatory_stats = {
            ...
            "cv": (
                np.mean(aleatory_stds) / np.mean(aleatory_means)
                if np.mean(aleatory_means) != 0
                else 0
            ),
        }
...
def create_default_two_layer_config(
    n_epistemic: int = 100,
    n_aleatory: int = 1000,
...
def create_epistemic_from_distribution(
    name: str, base_dist: Dict[str, Any], uncertainty_factor: float = 0.1
) -> EpistemicParameter:
    ...
    if dist_type in ["normal", "triangular"]:
        ...
        if "mean" in base_dist:
            ...
            raise ValueError(
                f"Cannot create epistemic parameter for {dist_type} without mean/mode"
            )
