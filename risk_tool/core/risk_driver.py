...
class RiskRegister:
    ...
    def get_applicable_risks(self, wbs_code: str, tags: List[str]) -> List[RiskItem]:
    ...
    def get_project_level_risks(self) -> List[RiskItem]:
        ...
        for risk in self.risks:
            ...
            if risk.applies_to:
                if any(
                    code in ["project", "indirects", "construction_management"]
...
class RiskSimulator:
    ...
    def __init__(
        self, risk_register: RiskRegister, random_state: np.random.RandomState
    ):
    ...
    def simulate_risk_impacts(
        self, n_samples: int, occurrences: Dict[str, np.ndarray]
    ) -> Dict[str, np.ndarray]:
...
class RiskAggregator:
    ...
    @staticmethod
    def aggregate_schedule_impacts(
        schedule_impacts: Dict[str, np.ndarray],
    ) -> np.ndarray:
    ...
    def calculate_risk_contributions(
        ...
        for risk in risk_register.risks:
            ...
            if risk.impact_mode == "multiplicative":
                ...
                contribution = (
                    correlation * np.std(impact) * np.std(risk_adjusted_total)
                )

            # Normalize by total variance
            variance_contribution = (
                (contribution**2) / total_variance if total_variance > 0 else 0
            )
            ...
            contributions[risk.id] = {
                ...
                "occurrence_rate": np.mean(
                    impact != (1.0 if risk.impact_mode == "multiplicative" else 0.0)
                ),
                "mean_impact_when_occurs": (
                    np.mean(
                        impact[
                            impact
                            != (1.0 if risk.impact_mode == "multiplicative" else 0.0)
    ...
    for i, risk_data in enumerate(risks):
        try:
            ...
            if not risk.applies_to and not risk.applies_by_tag:
                errors.append(
                    f"Risk {risk.id}: Must specify either applies_to or applies_by_tag"
                )
            ...
            if risk.schedule_days_dist:
                try:
                    ...
                    errors.append(
                        f"Risk {risk.id}: Invalid schedule distribution - {e}"
                    )
