...
class ConvergenceDiagnostics:
    ...
    def check_convergence(
        self, total_cost_samples: np.ndarray, current_iteration: int
    ) -> ConvergenceResult:
    ...
    @staticmethod
    def running_mean(
        samples: np.ndarray, window: int = 1000
    ...
    @staticmethod
    def running_percentile(
        samples: np.ndarray, percentile: float = 80, window: int = 1000
    ) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def convergence_test(
        samples: np.ndarray, confidence: float = 0.95, tolerance: float = 0.05
...
def validate_sampling_parameters(
    n_samples: int, n_dimensions: int, method: str
) -> None:
    ...
    if method.upper() == "LHS" and n_samples < n_dimensions:
        warnings.warn(
            f"LHS with {n_samples} samples and {n_dimensions} dimensions "
            "may not provide good space-filling properties"
        )


def optimize_sample_size(
    target_precision: float = 0.05, confidence: float = 0.95, estimated_cv: float = 0.3
) -> int:
