...
class Project(BaseModel):
    ...
    @field_validator("wbs")
    @classmethod
    def validate_wbs(cls, v: List[WBSItem]) -> List[WBSItem]:
        if not v:
...
class CostBreakdown:
    ...
    def calculate_category_breakdown(
        wbs_costs: Dict[str, np.ndarray], project: Project
    ) -> Dict[str, np.ndarray]:
    ...
    @staticmethod
    def calculate_contribution_to_variance(
        wbs_costs: Dict[str, np.ndarray], total_costs: np.ndarray
    ) -> Dict[str, float]:
...
class SimulationConfig(BaseModel):
    ...
    @classmethod
    def validate_sampling(cls, v: str) -> str:
        if v not in ["LHS", "MC"]:
...
def calculate_contingency_recommendation(
    total_costs: np.ndarray, base_cost: float, confidence_level: float = 0.8
) -> Dict[str, float]:
    ...
    return {
        ...
        "management_reserve_percent": (
            (management_reserve / base_cost * 100) if base_cost > 0 else 0
        ),
        "total_buffer": max(0, contingency + management_reserve),
        "total_buffer_percent": (
            ((contingency + management_reserve) / base_cost * 100)
            if base_cost > 0
            else 0
        ),
