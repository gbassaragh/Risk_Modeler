    ...
    def sample(self, size: int, random_state: np.random.RandomState) -> np.ndarray:
        ...
        for i in range(size):
            ...
            comp_sample = DistributionSampler.sample(
                self.component_samplers[comp_idx], 
                1, 
                random_state
            )
...
class GeneralizedParetoDistribution:
    ...
    def pdf(self, x: np.ndarray) -> np.ndarray:
        ...
        if abs(self.shape) < 1e-8:  # Exponential case
            ...
            if self.shape > 0:  # Finite upper bound
                ...
                z = z[valid_z]
                pdf_vals[valid][valid_z] = (1 / self.scale) * np.power(1 + self.shape * z, 
                                                                       -(1/self.shape + 1))
            else:  # No upper bound
                pdf_vals[valid] = (1 / self.scale) * np.power(1 + self.shape * z, 
                                                              -(1/self.shape + 1))
    ...
    def cdf(self, x: np.ndarray) -> np.ndarray:
        ...
        if abs(self.shape) < 1e-8:  # Exponential case
            ...
            if self.shape > 0:  # Finite upper bound
                valid_z = z < (1 / self.shape)
                z = z[valid_z]
                cdf_vals[valid][valid_z] = 1 - np.power(1 + self.shape * z, -1/self.shape)
    ...
    @classmethod
    def fit_peaks_over_threshold(cls, 
                                data: np.ndarray, 
                                threshold: float,
                                method: str = 'mle') -> 'GeneralizedParetoDistribution':
    ...
    @classmethod
    def _fit_mle(cls, exceedances: np.ndarray, threshold: float) -> 'GeneralizedParetoDistribution':
    ...
    @classmethod
    def _fit_pwm(cls, exceedances: np.ndarray, threshold: float) -> 'GeneralizedParetoDistribution':
...
class KDEDistribution:
    ...
    def __init__(self, data: np.ndarray, bandwidth: Optional[float] = None, 
        ...
        if bandwidth is None:
            ...
            bandwidth = 0.9 * min(std_data, iqr_data / 1.34) * np.power(self.n_samples, -0.2)
...
def create_kde_distribution(data: np.ndarray, config: Dict[str, Any]) -> KDEDistribution:
...
def create_empirical_distribution(data: np.ndarray, config: Dict[str, Any]) -> EmpiricalDistribution:
