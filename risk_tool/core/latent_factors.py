...
class LatentFactorModel:
    ...
    def __init__(self, factors: List[LatentFactor], 
    ...
    def _validate_model(self):
        ...
        try:
            ...
            if np.any(eigenvals < -1e-8):
                warnings.warn("Residual correlation matrix is not positive semi-definite")
        except:
        ...
        try:
            ...
            if np.any(eigenvals < -1e-8):
                warnings.warn("Implied correlation matrix is not positive semi-definite")
        except:
    ...
    def get_implied_correlation_matrix(self) -> np.ndarray:
        ...
        for i in range(self.n_variables):
            for j in range(self.n_variables):
                if i != j:
                    implied_cov[i, j] += self.residual_correlation_matrix[i, j] * np.sqrt(residual_variances[i] * residual_variances[j])
    ...
    def generate_correlated_samples(self, n_samples: int, 
        ...
        residual_samples = random_state.multivariate_normal(
            ...
            self.residual_correlation_matrix,
            size=n_samples
        )
...
class FactorModelBuilder:
    ...
    def add_factor(self, name: str, distribution_config: Dict[str, Any], 
                   variable_loadings: Dict[str, float]) -> 'FactorModelBuilder':
        """Add a latent factor.
    ...
    def add_residual_correlation(self, var1: str, var2: str, 
                                correlation: float) -> 'FactorModelBuilder':
...
def create_commodity_factor_model(commodity_variables: List[str],
                                 steel_loading: float = 0.8,
                                 aluminum_loading: float = 0.7,
                                 copper_loading: float = 0.6) -> LatentFactorModel:
    ...
    # Steel factor
    steel_vars = [v for v in commodity_variables if 'steel' in v.lower() or 'structure' in v.lower()]
    if steel_vars:
    ...
    # Aluminum factor
    aluminum_vars = [v for v in commodity_variables if 'aluminum' in v.lower() or 'conductor' in v.lower()]
    if aluminum_vars:
...
def create_weather_factor_model(weather_variables: List[str],
                               weather_loading: float = 0.7) -> LatentFactorModel:
...
def create_labor_market_factor_model(labor_variables: List[str],
                                   labor_loading: float = 0.6) -> LatentFactorModel:
