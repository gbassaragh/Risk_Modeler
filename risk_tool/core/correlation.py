...
class ImanConovierTransform:
    ...
    @staticmethod
    def transform(
        samples: np.ndarray, correlation_matrix: np.ndarray, method: str = "spearman"
    ) -> np.ndarray:
        ...
        if correlation_matrix.shape != (n_vars, n_vars):
            raise ValueError(
                "Correlation matrix size doesn't match number of variables"
            )
        ...
        try:
            ...
            correlation_matrix_pd = ImanConovierTransform._make_positive_definite(
                correlation_matrix
            )
        ...
        for i in range(n_vars):
            ...
            result[:, i] = np.interp(
                ranks_for_var, np.arange(n_samples), sorted_samples
            )
...
class CholekyTransform:
    ...
    def _regularize_matrix(
        matrix: np.ndarray, regularization: float = 1e-8
    ) -> np.ndarray:
...
class CorrelationValidator:
    ...
    def compute_correlation_matrix(
        samples: np.ndarray, method: str = "spearman"
    ) -> np.ndarray:
...
def apply_correlations(
    ...
    if method == "iman_conover":
        corr_method = (
            correlations[0].get("method", "spearman") if correlations else "spearman"
        )
