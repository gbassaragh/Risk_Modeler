...
class ValidationEngine:
    ...
    def validate_project(
        ...
        is_valid = len(self.errors) == 0 and (
            not self.fail_on_warnings or len(self.warnings) == 0
        )
        return is_valid, self.errors.copy(), self.warnings.copy()

    def validate_risks(
        self, risks_data: List[Dict[str, Any]]
    ) -> Tuple[bool, List[str], List[str]]:
        ...
        if not risks_data:
            self.warnings.append(
                "No risks provided - simulation will only include base uncertainty"
            )
        ...
        for i, corr_data in enumerate(correlations_data):
            try:
                ...
                if "pair" not in corr_data or "rho" not in corr_data:
                    self.errors.append(
                        f"Correlation {i}: Missing required fields 'pair' and 'rho'"
                    )
                    continue
                ...
                if not isinstance(pair, list) or len(pair) != 2:
                    self.errors.append(
                        f"Correlation {i}: 'pair' must be list of exactly 2 elements"
                    )
                    continue
                ...
                if not isinstance(rho, (int, float)) or abs(rho) > 1:
                    self.errors.append(
                        f"Correlation {i}: 'rho' must be number between -1 and 1"
                    )
                    continue
                ...
                if var1 not in available_vars:
                    self.warnings.append(
                        f"Correlation {i}: Variable '{var1}' not found in project/risks"
                    )

                if var2 not in available_vars:
                    self.warnings.append(
                        f"Correlation {i}: Variable '{var2}' not found in project/risks"
                    )

                # Validate correlation strength
                if abs(rho) > 0.9:
                    self.warnings.append(
                        f"Correlation {i}: Very strong correlation ({rho:.2f}) - ensure this is realistic"
                    )
    ...
    def validate_simulation_config(
        ...
        try:
            ...
            if not isinstance(iterations, int) or iterations <= 0:
                ...
                self.warnings.append(
                    "Less than 1000 iterations may give unstable results"
                )
            ...
            if "percentiles" in outputs:
                ...
                if not isinstance(percentiles, list) or not all(
                    0 <= p <= 100 for p in percentiles
                ):
                    self.errors.append(
                        "Percentiles must be list of numbers between 0 and 100"
                    )
            ...
            if "num_threads" in performance:
                ...
                if not isinstance(threads, int) or (threads != -1 and threads <= 0):
                    self.errors.append(
                        "num_threads must be positive integer or -1 for auto"
                    )
    ...
    def validate_file_access(
        ...
        for file_path in file_paths:
            ...
            if path.suffix.lower() not in supported_extensions:
                self.warnings.append(
                    f"File extension '{path.suffix}' may not be supported: {file_path}"
                )
            ...
            try:
                ...
                if file_size > 100 * 1024 * 1024:  # 100MB
                    self.warnings.append(
                        f"Large file ({file_size / 1024 / 1024:.1f} MB): {file_path}"
                    )
                elif file_size == 0:
        ...
        for i, item in enumerate(wbs_items):
            ...
            if item.dist_quantity:
                try:
                    ...
                    self.errors.append(
                        f"WBS {item.code}: Invalid quantity distribution - {e}"
                    )
            ...
            if item.dist_unit_cost:
                try:
                    ...
                    self.errors.append(
                        f"WBS {item.code}: Invalid unit cost distribution - {e}"
                    )
            ...
            if item.indirect_factor < 0 or item.indirect_factor > 2:
                self.warnings.append(
                    f"WBS {item.code}: Indirect factor {item.indirect_factor} seems unusual (0-2 typical)"
                )
        ...
        if base_year and target_year:
            try:
                ...
                if target < base:
                    ...
                    self.warnings.append(
                        "Escalation period > 20 years may be unrealistic"
                    )
            except ValueError:
    ...
    def _validate_project_business_logic(self, project: Project):
        ...
        for item in project.wbs:
            if any(
                tag in item.tags for tag in ["structures", "conductor", "foundations"]
            ):
    ...
    def _validate_risk_distributions(self, risk: RiskItem, index: int):
        ...
        try:
            ...
            self.errors.append(
                f"Risk {index} ({risk.id}): Invalid impact distribution - {e}"
            )
        ...
        if risk.schedule_days_dist:
            try:
                ...
                self.errors.append(
                    f"Risk {index} ({risk.id}): Invalid schedule distribution - {e}"
                )
    ...
    def _validate_risk_application(self, risk: RiskItem, index: int):
        ...
        if not risk.applies_to and not risk.applies_by_tag:
            self.errors.append(
                f"Risk {index} ({risk.id}): Must specify either applies_to or applies_by_tag"
            )

        if risk.applies_to and risk.applies_by_tag:
            self.warnings.append(
                f"Risk {index} ({risk.id}): Specifies both applies_to and applies_by_tag"
            )
    ...
    def _validate_risk_business_logic(self, risk: RiskItem, index: int):
        ...
        if risk.probability <= 0 or risk.probability > 1:
            self.errors.append(
                f"Risk {index} ({risk.id}): Probability must be between 0 and 1"
            )
        elif risk.probability > 0.9:
            self.warnings.append(
                f"Risk {index} ({risk.id}): Very high probability ({risk.probability}) - is this realistic?"
            )
        ...
        if risk.impact_mode == "multiplicative":
            ...
            if dist.get("type") == "pert":
                ...
                if min_val < 0.1 or max_val > 10:
                    self.warnings.append(
                        f"Risk {index} ({risk.id}): Extreme multiplicative impact range"
                    )
        else:  # additive
            ...
            if dist.get("type") == "pert":
                ...
                if max_val > 10000000:  # $10M
                    self.warnings.append(
                        f"Risk {index} ({risk.id}): Very large additive impact"
                    )
    ...
    def _validate_risk_portfolio(self, risks_data: List[Dict[str, Any]]):
        ...
        for category, count in categories.items():
            if count / total_risks > 0.6:
                self.warnings.append(
                    f"Risk concentration: {count}/{total_risks} risks in '{category}' category"
                )

        # High probability concentration
        if high_prob_risks / total_risks > 0.5:
            self.warnings.append(
                f"Many high-probability risks ({high_prob_risks}/{total_risks}) - validate realism"
            )
