...
class UncertaintyConfig:
    ...
    distribution_uncertainty: float = 0.15  # Default uncertainty in distribution parameters
...
class RiskModelUncertaintyAnalyzer:
    ...
    def _extract_epistemic_parameters(self) -> List[EpistemicParameter]:
        ...
        if hasattr(self.risk_model, 'correlation_matrix') and self.risk_model.correlation_matrix:
    ...
    def _create_distribution_epistemic_params(self) -> List[EpistemicParameter]:
        ...
        for item in self.risk_model.wbs_items:
            ...
            if item.dist_unit_cost:
                ...
                ep_param = self._create_dist_epistemic_param(
                    param_name, item.dist_unit_cost, self.config.distribution_uncertainty
                )
                if ep_param:
                    epistemic_params.append(ep_param)
        ...
        for item in self.risk_model.risk_items:
            if item.distribution:
                param_name = f"risk_{item.name}_{self._get_param_name(item.distribution)}"
                ep_param = self._create_dist_epistemic_param(
    ...
    def _create_dist_epistemic_param(self, 
                                    name: str, 
                                    distribution: Distribution, 
                                    uncertainty_factor: float) -> Optional[EpistemicParameter]:
    ...
    def _create_correlation_epistemic_params(self) -> List[EpistemicParameter]:
        ...
        for (var1, var2), corr_value in corr_matrix.correlations.items():
            if abs(corr_value) > 1e-6:  # Skip near-zero correlations
                ...
                epistemic_params.append(EpistemicParameter(param_name, epistemic_config))
    ...
    def _aleatory_model_function(self, 
    ...
    def _create_modified_risk_model(self, epistemic_params: Dict[str, float]) -> RiskModel:
        """Create modified risk model with epistemic parameter values."""
        ...
        for item in self.risk_model.risk_items:
            ...
            if item.distribution:
                param_key = f"risk_{item.name}_{self._get_param_name(item.distribution)}"
                if param_key in epistemic_params:
    ...
    def _update_correlations(self, epistemic_params: Dict[str, float]) -> Optional[CorrelationMatrix]:
        """Update correlation matrix with epistemic parameters."""
        if not hasattr(self.risk_model, 'correlation_matrix') or not self.risk_model.correlation_matrix:
            return None
    ...
    def _update_factor_model(self, epistemic_params: Dict[str, float]) -> Optional[LatentFactorModel]:
        """Update factor model with epistemic parameters."""
        if not hasattr(self.risk_model, 'factor_model') or not self.risk_model.factor_model:
            return None
        ...
        return LatentFactorModel(
            updated_factors, 
            original_model.residual_correlations
        )


# Convenience functions

def analyze_project_uncertainty(project_config: ProjectConfig,
                               wbs_items: List[WBSItem],
                               risk_items: List[RiskItem],
                               uncertainty_config: UncertaintyConfig = None) -> TwoLayerResults:
...
def quick_uncertainty_analysis(risk_model: RiskModel,
                              n_epistemic: int = 50,
                              n_aleatory: int = 2000) -> TwoLayerResults:
