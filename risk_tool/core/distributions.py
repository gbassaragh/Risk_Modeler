...
class Normal(DistributionConfig):
    ...
    @field_validator("stdev")
    @classmethod
    def validate_stdev(cls, v: float) -> float:
        if v <= 0:
...
class LogNormal(DistributionConfig):
    ...
    @classmethod
    def validate_mean(cls, v: float) -> float:
        if v <= 0:
...
class Discrete(DistributionConfig):
    ...
    @field_validator("pmf")
    @classmethod
    def validate_pmf(cls, v: List[List[float]]) -> List[List[float]]:
        if not v:
...
class DistributionSampler:
    ...
    @staticmethod
    def sample(
        dist_config: Dict[str, Any], size: int, random_state: np.random.RandomState
    ) -> np.ndarray:
        ...
        if dist_type == "triangular":
            return DistributionSampler._sample_triangular(
                dist_config, size, random_state
            )
        elif dist_type == "pert":
            ...
            return DistributionSampler._sample_lognormal(
                dist_config, size, random_state
            )
            ...
            return DistributionSampler._sample_empirical(
                dist_config, size, random_state
            )
    ...
    def _sample_triangular(
        config: Dict[str, Any], size: int, rs: np.random.RandomState
    ) -> np.ndarray:
        ...
        return stats.triang.rvs(
            c, loc=low, scale=high - low, size=size, random_state=rs
        )
    ...
    def _sample_normal(
        ...
        if truncate_low is not None or truncate_high is not None:
            ...
            return stats.truncnorm.rvs(
                a, b, loc=mean, scale=stdev, size=size, random_state=rs
            )
        else:
