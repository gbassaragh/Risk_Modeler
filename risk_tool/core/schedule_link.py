...
class ScheduleSimulator:
    ...
    def calculate_project_duration(
        self, activity_durations: Dict[str, np.ndarray]
        ...
        for activity in sorted_activities:
            ...
            if activity.predecessors:
                ...
                if predecessor_finish_times:
                    start_times[activity_id] = np.maximum.reduce(
                        predecessor_finish_times
                    )

            # Finish time = start time + duration
            finish_times[activity_id] = (
                start_times[activity_id] + activity_durations[activity_id]
            )
...
class ScheduleCostCalculator:
    ...
    def calculate_indirect_costs(
        project_durations: np.ndarray, indirects_per_day: float
    ) -> np.ndarray:
    ...
    @staticmethod
    def calculate_delay_costs(
        base_duration: float, actual_durations: np.ndarray, delay_cost_per_day: float
    ) -> np.ndarray:
    ...
    @staticmethod
    def calculate_acceleration_costs(
        base_duration: float,
        actual_durations: np.ndarray,
        acceleration_cost_curve: Dict[str, float],
    ) -> np.ndarray:
        ...
        for i in range(n_samples):
            ...
            if duration < base_duration:
                ...
                acceleration_costs[i] = (
                    accel_pct * cost_multiplier * base_duration * 1000
                )  # Example cost
...
class OutageConstraints:
    ...
    @staticmethod
    def apply_outage_constraints(
        activity_durations: Dict[str, np.ndarray],
...
def create_default_schedule() -> Schedule:
    ...
    activities = [
        Activity(
            ...
            duration_dist={"type": "triangular", "low": 60, "mode": 90, "high": 120},
        ),
        Activity(
            id="procurement",
            name="Procurement",
            duration_dist={"type": "pert", "min": 90, "most_likely": 180, "max": 270},
            predecessors=["engineering"],
        ),
        Activity(
            id="construction",
            name="Construction",
            duration_dist={"type": "pert", "min": 120, "most_likely": 200, "max": 300},
            predecessors=["engineering", "procurement"],
        ),
        Activity(
            id="commissioning",
            name="Testing & Commissioning",
            duration_dist={"type": "triangular", "low": 20, "mode": 30, "high": 45},
    ...
    try:
        ...
        for activity in schedule.activities:
            try:
                ...
                errors.append(
                    f"Activity {activity.id}: Invalid duration distribution - {e}"
                )
        ...
        for activity in schedule.activities:
            for pred_id in activity.predecessors:
                if pred_id not in activity_ids:
                    errors.append(
                        f"Activity {activity.id}: Unknown predecessor {pred_id}"
                    )
