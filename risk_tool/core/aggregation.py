...
from .cost_models import (
    ...
    CostBreakdown,
    SimulationConfig,
    calculate_contingency_recommendation,
)
from .risk_driver import (
    RiskRegister,
    RiskSimulator,
    RiskAggregator,
    RiskAnalyzer,
...
class SimulationEngine:
    ...
    def run_simulation(
        ...
        try:
            ...
            if correlations:
                base_samples = self._apply_correlations(
                    base_samples, correlations, project, risks
                )
            ...
            if schedule:
                final_costs = self._apply_schedule_effects(
                    escalated_costs, schedule, project
                )
            else:
            ...
            results["iterations_per_second"] = (
                self.config.iterations / results["simulation_time"]
            )
    ...
    def _generate_base_samples(
        self, project: Project, risks: Optional[List[RiskItem]]
    ) -> Dict[str, np.ndarray]:
    ...
    def _convert_uniform_to_distribution(
        self, uniform_samples: np.ndarray, dist_config: Dict[str, Any]
    ) -> np.ndarray:
        ...
        if dist_type == "triangular":
            ...
            return DistributionSampler.sample(
                dist_config, len(uniform_samples), self.random_state
            )
    ...
    def _simulate_costs(
        self, project: Project, samples: Dict[str, np.ndarray]
    ) -> Dict[str, np.ndarray]:
    ...
    def _simulate_risks(
        ...
        for risk in risks:
            # Get risk occurrence
            risk_occurs = self.random_state.binomial(
                1, risk.probability, self.config.iterations
            ).astype(bool)
        ...
        if np.any(project_level_adjustments != 0):
            ...
            for code in risk_adjusted_costs:
                base_proportion = np.mean(wbs_costs[code]) / sum(
                    np.mean(costs) for costs in wbs_costs.values()
                )
    ...
    def _apply_escalation(
        self, costs: Dict[str, np.ndarray], project: Project
    ) -> Dict[str, np.ndarray]:
    ...
    def _apply_schedule_effects(
        self, costs: Dict[str, np.ndarray], schedule: Schedule, project: Project
    ) -> Dict[str, np.ndarray]:
        """Apply schedule-driven cost effects."""
        # Simulate schedule
        schedule_simulator = ScheduleSimulator(schedule, self.random_state)
        activity_durations = schedule_simulator.simulate_durations(
            self.config.iterations
        )
        project_durations, _ = schedule_simulator.calculate_project_duration(
            activity_durations
        )
