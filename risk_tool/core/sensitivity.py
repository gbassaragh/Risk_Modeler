...
class SensitivityAnalyzer:
    ...
    def calculate_tornado_diagram(
        ...
        for name, values in input_variables.items():
            ...
            if method == "spearman":
                ...
                sensitivity = (
                    SensitivityAnalyzer._calculate_standardized_regression_coefficient(
    ...
    @staticmethod
    def _calculate_standardized_regression_coefficient(
    ...
    def calculate_variance_decomposition(
        input_variables: Dict[str, np.ndarray], output_variable: np.ndarray
    ) -> Dict[str, float]:
        ...
        for name, values in input_variables.items():
            ...
            contribution = (
                (correlation * np.std(values) * np.std(output_variable)) ** 2
            ) / total_variance
    ...
    def calculate_sobol_indices(
        ...
        if total_variance == 0:
            return {
                name: {"S1": 0.0, "confidence_low": 0.0, "confidence_high": 0.0}
...
class RiskContributionAnalyzer:
    ...
    def analyze_risk_contributions(
        ...
        for risk_id, impact_values in individual_risk_impacts.items():
            ...
            if total_variance > 0:
                ...
                contribution = (
                    correlation * np.std(impact_values) * np.std(risk_adjusted_costs)
                ) / total_variance
            ...
            if metadata.get("impact_mode") == "multiplicative":
                ...
                if np.any(occurred):
                    contribution_metrics["conditional_mean_impact"] = np.mean(
                        impact_values[occurred]
                    )
    ...
    @staticmethod
    def rank_risks_by_contribution(
        ...
        for risk_id, metrics in risk_contributions.items():
            value = abs(
                metrics.get(ranking_method, 0.0)
            )  # Use absolute value for correlation
