...
class ExcelImporter:
    ...
    def import_risks(self, file_path: str, sheet_name: str = 'Risks') -> List[Dict[str, Any]]:
    ...
    def _process_wbs_row(self, row: pd.Series, column_mapping: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            dist_columns = [col for col in row.index if 'dist_' in col or 'distribution' in col]
    ...
    def _process_risk_row(self, row: pd.Series, column_mapping: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            risk_item = {
                ...
                'impact_dist': {'type': 'triangular', 'low': 1.0, 'mode': 1.1, 'high': 1.3}  # Default
            }
            ...
            if 'applies_to' in column_mapping and not pd.isna(row[column_mapping['applies_to']]):
                ...
                risk_item['applies_to'] = [item.strip() for item in applies_str.split(',')]
            ...
            if 'applies_by_tag' in column_mapping and not pd.isna(row[column_mapping['applies_by_tag']]):
                ...
                risk_item['applies_by_tag'] = [tag.strip() for tag in tags_str.split(',')]
            
            # Process impact distribution
            impact_columns = [col for col in row.index if 'impact' in col and ('dist' in col or 'min' in col or 'max' in col)]
            if impact_columns:
    ...
    def _parse_distribution_spec(self, spec: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if 'triangular' in spec.lower():
                ...
                import re
                match = re.search(r'triangular\s*\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*\)', spec)
                if match:
                    return {
                        'type': 'triangular',
                ...
                import re
                match = re.search(r'pert\s*\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*\)', spec)
                if match:
                    return {
                        'type': 'pert',
    ...
    def _parse_impact_distribution(self, row: pd.Series, impact_columns: List[str]) -> Optional[Dict[str, Any]]:
        """Parse impact distribution from row data."""
        try:
            # Look for standard distribution columns
            min_col = next((col for col in impact_columns if 'min' in col.lower()), None)
            mode_col = next((col for col in impact_columns if ('mode' in col.lower() or 'most_likely' in col.lower())), None)
            max_col = next((col for col in impact_columns if 'max' in col.lower()), None)
            ...
            low_col = next((col for col in impact_columns if 'low' in col.lower()), None)
            high_col = next((col for col in impact_columns if 'high' in col.lower()), None)
...
class ExcelExporter:
            ...
            with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                ...
                if 'simulation_data' in results_dict:
                    ...
                    if isinstance(sim_data, dict) and 'total_costs' in sim_data:
                        ...
                        sample_df.to_excel(writer, sheet_name='Simulation_Sample', index=False)
    ...
    def create_wbs_template(self, file_path: str) -> None:
        ...
        try:
            ...
            # Headers
            headers = [
                'code', 'name', 'quantity', 'uom', 'unit_cost', 
                'tags', 'indirect_factor', 'dist_quantity', 'dist_unit_cost'
            ]
            ...
            # Example data
            example_data = [
                ['1.1', 'ROW Clearing & Access', 10.0, 'miles', 120000.0, 'TL,access,weather_sensitive', 0.12, 'triangular(9,10,12)', 'pert(100000,120000,160000)'],
                ['2.1', 'Steel Structures', 50, 'each', 25000.0, 'structures,steel', 0.08, '', 'lognormal(25000,0.2)'],
                ['2.2', 'Foundations', 50, 'each', 15000.0, 'foundations,concrete', 0.10, '', 'triangular(12000,15000,20000)'],
                ['2.3', 'Conductor & OPGW', 60.0, 'miles-conductor', 85000.0, 'materials,commodity_aluminum', 0.05, '', 'lognormal(85000,0.15)']
            ]
    ...
    def create_risk_template(self, file_path: str) -> None:
        ...
        try:
            ...
            # Headers
            headers = [
                'id', 'title', 'category', 'probability', 'impact_mode',
                'impact_min', 'impact_most_likely', 'impact_max',
                'applies_to', 'applies_by_tag', 'schedule_days_min', 'schedule_days_max'
            ]
            ...
            example_data = [
                ['R-ENV-001', 'Wetland matting extent > plan', 'Environmental/Access', 0.45, 'multiplicative', 1.03, 1.12, 1.35, '1.1', '', 0, 20],
                ['R-PROC-002', 'Steel pole procurement volatility', 'Commodity/Market', 0.35, 'multiplicative', 0.9, 1.0, 1.3, '', 'steel,structures', 0, 0],
                ['R-OUT-003', 'Outage window constraints', 'Operations', 0.25, 'additive', 200000, 400000, 1200000, '', 'indirects', 0, 30],
                ['R-WX-004', 'Storm season productivity loss', 'Weather', 0.60, 'multiplicative', 1.1, 1.25, 1.5, '', 'weather_sensitive', 5, 15]
            ]
