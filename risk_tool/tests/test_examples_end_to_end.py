...
class TestTransmissionLineExample:
    ...
    def test_transmission_line_full_simulation(self, transmission_line_project, 
        ...
        base_cost = sum(item['base_cost'] for item in transmission_line_project['wbs_items'])
        ...
        if weather_contrib:
            assert weather_contrib.get('mean_impact', 0) > 100000  # Should have significant impact
    
    def test_transmission_line_validation(self, transmission_line_project, 
                                        transmission_line_risks, 
                                        transmission_line_correlations):
    ...
    def test_transmission_line_contingency_calculation(self, transmission_line_project, 
                                                     transmission_line_risks, 
                                                     transmission_line_correlations):
        ...
        assert 10 <= contingency_percent <= 60, f"Contingency {contingency_percent:.1f}% seems unrealistic"
...
class TestSubstationExample:
    ...
    def test_substation_simulation(self, substation_project, substation_risks):
        ...
        results = run_simulation(substation_project, substation_risks, config_data=config)
        ...
        contingency_percent = ((results.total_cost_statistics['p80'] - base_cost) / base_cost) * 100
...
class TestCLIIntegration:
    ...
    def test_cli_run_command(self, sample_project_files):
        ...
        result = runner.invoke(app, [
            ...
            '--risks', str(sample_project_files['risks']),
            '--config', str(sample_project_files['config']),
...
class TestPerformanceBenchmarks:
        ...
        risks = [
            {
                ...
                'affected_wbs': [f'wbs_{j}' for j in range(min(3, 10))]  # Affect first 3 WBS items
            }
    ...
    def test_memory_efficiency(self):
        ...
        config = {
            ...
            'reporting': {'percentiles': [50], 'enable_charts': False, 'sensitivity_analysis': False}
        }
...
class TestEdgeCasesEndToEnd:
    ...
    def test_project_with_perfect_correlations(self):
        ...
        project = {
            'project_info': {'name': 'Perfect Correlation Test', 'category': 'transmission_line'},
            'wbs_items': [
                {
                    'id': 'item1',
                    'name': 'Item 1',
        ...
        results = run_simulation(project, correlations_data=correlations, config_data=config)
