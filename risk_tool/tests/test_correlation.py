...
class TestImanConovierTransform:
    ...
    def test_multiple_correlations(self, random_samples):
        ...
        n_vars = random_samples.shape[1]
        target_matrix = np.array([[1.0, 0.5, 0.3], [0.5, 1.0, 0.2], [0.3, 0.2, 1.0]])
    ...
    def test_non_symmetric_matrix(self, random_samples):
        ...
        with pytest.raises(ValueError):
            ImanConovierTransform.transform(random_samples, non_symmetric, "spearman")
...
class TestCholekyTransform:
    ...
    def test_normal_transformation(self, normal_samples):
        ...
        target_matrix = np.array([[1.0, 0.7], [0.7, 1.0]])

        transformed = CholekyTransform.transform_normal(normal_samples, target_matrix)
    ...
    def test_singular_matrix_regularization(self, normal_samples):
        ...
        singular_matrix = np.array(
            [[1.0, 1.0], [1.0, 1.0]]  # Perfectly correlated - singular
        )

        # Should handle gracefully (regularize)
        transformed = CholekyTransform.transform_normal(normal_samples, singular_matrix)
...
class TestApplyCorrelations:
    ...
    def test_no_correlations(self, independent_samples):
        ...
        result = apply_correlations(independent_samples, correlations, variable_names)
    ...
    def test_single_correlation(self, independent_samples):
        ...
        correlations = [{"pair": ["var1", "var2"], "rho": 0.5, "method": "spearman"}]
        variable_names = ["var1", "var2", "var3"]
    ...
    def test_multiple_correlations(self, independent_samples):
        """Test with multiple correlations."""
        correlations = [
            {"pair": ["var1", "var2"], "rho": 0.6, "method": "spearman"},
            {"pair": ["var1", "var3"], "rho": 0.3, "method": "spearman"},
        ]
        variable_names = ["var1", "var2", "var3"]
    ...
    def test_invalid_variable_name(self, independent_samples):
        ...
        correlations = [
            {"pair": ["var1", "unknown_var"], "rho": 0.5, "method": "spearman"}
        ]
        variable_names = ["var1", "var2", "var3"]

        with pytest.raises(ValueError):
            apply_correlations(independent_samples, correlations, variable_names)
...
class TestCorrelationEdgeCases:
    ...
    def test_small_sample_size(self):
        ...
        result = ImanConovierTransform.transform(
            small_samples, target_matrix, "spearman"
        )
