...
class TestCLI:
    ...
    def test_template_substation_excel(self, runner, temp_dir):
        ...
        result = runner.invoke(
            app, ["template", "substation", "excel", str(output_file)]
        )
    ...
    def test_template_csv_format(self, runner, temp_dir):
        ...
        result = runner.invoke(
            app, ["template", "transmission_line", "csv", str(output_file)]
        )
    ...
    def test_template_invalid_category(self, runner, temp_dir):
        ...
        result = runner.invoke(
            app, ["template", "invalid_category", "json", str(output_file)]
        )
    ...
    def test_template_invalid_format(self, runner, temp_dir):
        ...
        result = runner.invoke(
            app, ["template", "transmission_line", "xyz", str(output_file)]
        )
    ...
    def test_validate_command_success(self, mock_validate, runner, temp_dir):
        ...
        if result.exit_code == 0:
            ...
            assert any(
                word in result.stdout.lower() for word in ["success", "valid", "passed"]
            )
        else:
    ...
    def test_validate_command_failure(self, mock_validate, runner, temp_dir):
        ...
        # Create a test JSON file
        test_data = {"project_info": {"name": "Test"}, "wbs_items": []}
        ...
        if mock_validate.called:
            ...
            assert (
                len(result.stdout) > 0
            ), "Failed validation should provide error details"
            assert any(
                word in result.stdout.lower() for word in ["error", "failed", "invalid"]
            )
    ...
    def test_run_command_basic(self, mock_run_sim, runner, temp_dir):
        ...
        project_data = {
            ...
            "simulation_config": {"iterations": 1000, "random_seed": 42},
        }
        ...
        mock_results.total_cost_statistics = {
            "mean": 100000,
            "p50": 98000,
            "p80": 105000,
        }
...
class TestCLIIntegration:
    ...
    def test_complete_workflow_validation(self, runner, temp_dir):
        ...
        result1 = runner.invoke(
            app, ["template", "transmission_line", "json", str(template_file)]
        )
        ...
        for item in template_data["wbs_items"]:
            ...
            assert (
                "unit_cost" in item and item["unit_cost"] > 0
            ), "WBS items must have positive costs"
    ...
    def test_output_completeness(self, runner, temp_dir):
        ...
        result = runner.invoke(
            app, ["template", "substation", "json", str(template_file)]
        )
        ...
        result = runner.invoke(
            app, ["template", "transmission_line", "csv", str(csv_file)]
        )
    ...
    def test_different_format_consistency(self, runner, temp_dir):
        ...
        result1 = runner.invoke(
            app, ["template", "transmission_line", "json", str(json_file)]
        )
        assert result1.exit_code == 0
...
class TestProductionReadiness:
        ...
        for item in data["wbs_items"]:
            ...
            if "dist_unit_cost" in item:
                ...
                assert dist["type"] in [
                    ...
                    "pert",
                    "uniform",
                ], "Real distribution type required"
    ...
    def test_complete_error_context(self, runner, temp_dir):
        ...
        error_scenarios = [
            (
                ["template", "invalid_type", "json", "/tmp/test.json"],
                "invalid category",
            ),
            (
                ["template", "transmission_line", "invalid_format", "/tmp/test"],
                "invalid format",
            ),
        ...
        for command_args, error_type in error_scenarios:
            ...
            assert (
                len(result.stdout) > 0
            ), f"Should provide error message for {error_type}"
            # Should not just return exit code without explanation
            assert (
                not result.stdout.isspace()
            ), f"Error message should not be just whitespace for {error_type}"
    ...
    def test_authentication_and_security_readiness(self, runner, temp_dir):
        ...
        for issue in security_issues:
            assert (
                issue not in content
            ), f"Template should not contain '{issue}' in production"
        ...
        for artifact in dev_artifacts:
            ...
            ), f"Template should not contain development artifact '{artifact}'"
