...
class TestSimulationEngine:
    ...
    def test_minimal_simulation_runs(self, minimal_project, basic_config):
        ...
        assert results.total_cost_statistics['p80'] > results.total_cost_statistics['p50']
        assert results.total_cost_statistics['p90'] > results.total_cost_statistics['p80']
    
    def test_simulation_with_risks(self, minimal_project, basic_config, sample_risks):
        """Test simulation with risk data."""
        results = run_simulation(
            minimal_project, 
            risks_data=sample_risks,
            config_data=basic_config
        )
    ...
    def test_simulation_determinism(self, minimal_project, basic_config):
        ...
        np.testing.assert_array_equal(results1.total_cost_samples, results2.total_cost_samples)
        
        for wbs_id in results1.wbs_cost_samples:
            np.testing.assert_array_equal(
                results1.wbs_cost_samples[wbs_id],
                results2.wbs_cost_samples[wbs_id]
            )
    ...
    def test_simulation_different_seeds(self, minimal_project, basic_config):
        ...
        assert not np.array_equal(results1.total_cost_samples, results2.total_cost_samples)
    
    def test_simulation_sensitivity_analysis(self, minimal_project, basic_config, sample_risks):
...
class TestSimulationEdgeCases:
    ...
    def test_simulation_with_single_wbs_item(self, basic_config):
        ...
        np.testing.assert_array_equal(
            results.total_cost_samples,
            results.wbs_cost_samples['single_item']
        )
