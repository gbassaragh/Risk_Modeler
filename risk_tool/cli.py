...
app: typer.Typer = typer.Typer(
    help="Monte Carlo risk modeling tool for utility T&D projects"
)
...
def run(
    project: str = typer.Option(
        ..., "--project", "-p", help="Project file (Excel, CSV, or JSON)"
    ),
    risks: Optional[str] = typer.Option(
        None, "--risks", "-r", help="Risk file (Excel, CSV, or JSON)"
    ),
    correlations: Optional[str] = typer.Option(
        None, "--correlations", "-c", help="Correlations file (CSV or JSON)"
    ),
    config: Optional[str] = typer.Option(
        None, "--config", help="Configuration file (JSON or YAML)"
    ),
    output: str = typer.Option("./results", "--output", "-o", help="Output directory"),
    iterations: Optional[int] = typer.Option(
        None, "--iterations", "-n", help="Number of iterations"
    ),
    seed: Optional[int] = typer.Option(
        None, "--seed", help="Random seed for reproducibility"
    ),
    sampling: Optional[str] = typer.Option(
        None, "--sampling", help="Sampling method (LHS or MC)"
    ),
    validate_only: bool = typer.Option(
        False, "--validate-only", help="Only validate inputs, don't run simulation"
    ),
    export_formats: Optional[List[str]] = typer.Option(
        None, "--formats", help="Export formats (csv,xlsx,json,pdf)"
    ),
    ...
    try:
        ...
        correlations_data = (
            _load_correlations_data(correlations) if correlations else None
        )
        ...
        if not is_valid:
            console.print(
                "[red]❌ Validation failed. Please fix errors before running simulation.[/red]"
            )
            raise typer.Exit(1)
        ...
        # Run simulation
        console.print(
            f"[yellow]Running simulation with {config_data.get('iterations', 50000):,} iterations...[/yellow]"
        )
...
def template(
    category: str = typer.Argument(
        ..., help="Project category (transmission_line, substation)"
    ),
    format: str = typer.Argument(..., help="Output format (json, excel, csv)"),
    output_file: str = typer.Argument(..., help="Output file path"),
):
    """Generate project templates.

    Create comprehensive project templates for transmission lines or substations
    with WBS items, correlations, and simulation configuration.
    """

    console.print(
        f"[yellow]Generating {category} template in {format} format...[/yellow]"
    )
    ...
    try:
        ...
        if category == "transmission_line":
            if format == "excel":
                generator.create_transmission_line_excel(output_path)
            elif format == "json":
                # Copy the pre-created JSON template
                template_path = (
                ...
                if template_path.exists():
                    with open(template_path) as src, open(output_path, "w") as dst:
                        dst.write(src.read())
                else:
                    raise FileNotFoundError("Transmission line JSON template not found")
            elif format == "csv":
                # Copy the CSV template
                template_path = (
                    Path(__file__).parent / "templates" / "transmission_line_wbs.csv"
                )
                if template_path.exists():
                    with open(template_path) as src, open(output_path, "w") as dst:
                        dst.write(src.read())
                else:
                    raise FileNotFoundError("Transmission line CSV template not found")
            else:
                raise ValueError(
                    f"Format '{format}' not supported. Use: json, excel, csv"
                )

        elif category == "substation":
            if format == "excel":
                generator.create_substation_excel(output_path)
            elif format == "json":
                template_path = (
                    Path(__file__).parent / "templates" / "substation_template.json"
                )
                if template_path.exists():
                    with open(template_path) as src, open(output_path, "w") as dst:
                        dst.write(src.read())
                else:
                    raise FileNotFoundError("Substation JSON template not found")
            elif format == "csv":
                template_path = (
                    Path(__file__).parent / "templates" / "substation_wbs.csv"
                )
                if template_path.exists():
                    with open(template_path) as src, open(output_path, "w") as dst:
                        dst.write(src.read())
                else:
                    raise FileNotFoundError("Substation CSV template not found")
            else:
                raise ValueError(
                    f"Format '{format}' not supported. Use: json, excel, csv"
                )
            ...
            raise ValueError(
                f"Category '{category}' not supported. Use: transmission_line, substation"
            )

        console.print(f"[green]✅ Template created: {output_path.absolute()}[/green]")
        console.print(f"[blue]📁 Ready to customize with your project data[/blue]")
...
def validate(
    ...
    correlations: Optional[str] = typer.Option(
        None, "--correlations", "-c", help="Correlations file"
    ),
...
def report(
    ...
    format: str = typer.Option(
        "html", "--format", "-f", help="Report format (html, pdf, json)"
    ),
...
def verify(
    ...
    try:
        ...
        if verification_result["reproducible"]:
            console.print(
                f"[green]✅ Simulation is reproducible across {runs} runs[/green]"
            )
...
def info(
    project: Optional[str] = typer.Option(
        None, "--project", "-p", help="Project file to analyze"
    )
):
...
def _display_results_summary(results, simulation_time: float, verbose: bool):
    ...
    if results.contingency_recommendations:
        ...
        table.add_row("", "")  # Separator
        table.add_row(
            "Recommended Contingency", f"${contingency.get('contingency', 0):,.0f}"
        )
        table.add_row(
            "Contingency %", f"{contingency.get('contingency_percent', 0):.1f}%"
        )
