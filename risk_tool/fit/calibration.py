...
class DistributionFitter:
    ...
    def _validate_distribution(self, distribution: str) -> None:
        ...
        if distribution not in self.SUPPORTED_DISTRIBUTIONS:
            ...
            raise ValueError(
                f"Unsupported distribution: {distribution}. " f"Supported: {supported}"
            )

    def _calculate_information_criteria(
        self, log_likelihood: float, n_params: int
    ) -> Tuple[float, float]:
...
class MLEFitter(DistributionFitter):
    ...
    def _fit_normal(self) -> CalibrationResults:
        ...
        ks_stat, ks_p = stats.kstest(
            self.data, lambda x: stats.norm.cdf(x, loc=loc, scale=scale)
        )
    ...
    def _fit_lognormal(self) -> CalibrationResults:
        ...
        # Goodness of fit
        ks_stat, ks_p = stats.kstest(
            self.data, lambda x: stats.lognorm.cdf(x, s=s, scale=scale)
        )
    ...
    def _fit_triangular(self) -> CalibrationResults:
        ...
        log_likelihood = np.sum(
            stats.triang.logpdf(self.data, c=c_param, loc=loc, scale=scale)
        )
        ...
        # Goodness of fit
        ks_stat, ks_p = stats.kstest(
            self.data, lambda x: stats.triang.cdf(x, c=c_param, loc=loc, scale=scale)
        )
    ...
    def _fit_beta(self) -> CalibrationResults:
        ...
        # Goodness of fit
        ks_stat, ks_p = stats.kstest(
            data_scaled, lambda x: stats.beta.cdf(x, a=alpha, b=beta)
        )
    ...
    def _fit_scipy_generic(self, distribution: str) -> CalibrationResults:
        ...
        try:
            ...
            aic, bic = self._calculate_information_criteria(
                log_likelihood, len(fitted_params)
            )

            # Goodness of fit
            ks_stat, ks_p = stats.kstest(
                self.data, lambda x: scipy_dist.cdf(x, *fitted_params)
            )
    ...
    def _fit_uniform(self) -> CalibrationResults:
        ...
        # Goodness of fit
        ks_stat, ks_p = stats.kstest(
            self.data, lambda x: stats.uniform.cdf(x, loc=loc, scale=scale)
        )
    ...
    def _fit_exponential(self) -> CalibrationResults:
        ...
        # Goodness of fit
        ks_stat, ks_p = stats.kstest(
            self.data, lambda x: stats.expon.cdf(x, scale=scale)
        )
...
class BayesianFitter(DistributionFitter):
    ...
    def _fit_normal_bayesian(
        self, n_samples, n_tune, chains, **kwargs
    ) -> CalibrationResults:
        ...
        intervals = {
            "loc": (
                float(summary.loc["mu", "hdi_5%"]),
                float(summary.loc["mu", "hdi_95%"]),
            ),
            "scale": (
                float(summary.loc["sigma", "hdi_5%"]),
                float(summary.loc["sigma", "hdi_95%"]),
            ),
        }
        ...
        log_likelihood = np.sum(
            stats.norm.logpdf(
                self.data, loc=parameters["loc"], scale=parameters["scale"]
            )
        )
        ...
        convergence_info = {
            ...
            "eff_sample_size": {
                var: float(summary.loc[var, "ess_bulk"]) for var in ["mu", "sigma"]
            },
    ...
    def _fit_lognormal_bayesian(
        ...
        parameters = {"s": sigma_mean, "scale": np.exp(mu_mean)}
        ...
        log_likelihood = np.sum(
            stats.lognorm.logpdf(
                self.data, s=parameters["s"], scale=parameters["scale"]
            )
        )
    ...
    def _fit_gamma_bayesian(
        ...
        convergence_info = {
            "r_hat": {
                var: float(summary.loc[var, "r_hat"]) for var in ["alpha", "beta"]
            },
            "eff_sample_size": {
                var: float(summary.loc[var, "ess_bulk"]) for var in ["alpha", "beta"]
            },
            "n_divergences": int(posterior.sample_stats.diverging.sum()),
        }
...
class GoodnessOfFitTester:
    ...
    def kolmogorov_smirnov_test(
        data: np.ndarray, distribution: str, parameters: Dict[str, float]
    ) -> Tuple[float, float]:
...
def fit_distribution_bayesian(
    data: np.ndarray, distribution: str, n_samples: int = 2000
) -> CalibrationResults:
...
def compare_distributions(
    data: np.ndarray, distributions: List[str]
) -> List[CalibrationResults]:
