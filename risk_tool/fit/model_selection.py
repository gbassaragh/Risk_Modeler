...
class ModelComparisonResult:
    ...
    def summary(self) -> str:
        ...
        lines.append(
            f"Best score: {self.ranking_scores[self.best_model.distribution_type]:.2f}"
        )
...
class CrossValidationResult:
    ...
    def summary(self) -> str:
        ...
        lines.append(
            f"Mean Log-Likelihood: {self.mean_log_likelihood:.2f} ± {self.std_log_likelihood:.2f}"
        )
        ...
        if self.mean_pred_error is not None:
            lines.append(
                f"Mean Prediction Error: {self.mean_pred_error:.4f} ± {self.std_pred_error:.4f}"
            )
...
class InformationCriteria:
    ...
    @staticmethod
    def compare_models(
        results: List[InformationCriteriaResult],
    ) -> List[InformationCriteriaResult]:
...
class CrossValidator:
    ...
    def __init__(
        self, cv_method: str = "kfold", n_splits: int = 5, random_state: int = 42
    ):
    ...
    def validate_distribution(
        ...
        if len(data) < self.n_splits * 5:
            warnings.warn(
                f"Small dataset for {self.n_splits}-fold CV. Results may be unreliable."
            )
        ...
        for fold, (train_idx, test_idx) in enumerate(self.cv.split(data)):
            ...
            try:
                ...
                if method == "mle":
                    ...
                    result = fitter.fit(
                        distribution, n_samples=1000
                    )  # Fewer samples for CV
                else:
                ...
                test_ll = self._evaluate_on_test_data(
                    test_data, distribution, result.parameters
                )
                ...
                pred_error = self._calculate_predictive_error(
                    test_data, distribution, result.parameters
                )
    ...
    def _evaluate_on_test_data(
        self, test_data: np.ndarray, distribution: str, parameters: Dict[str, float]
    ) -> float:
...
class ModelSelector:
    ...
    def __init__(self, selection_criteria: List[str] = None, cv_enabled: bool = True):
    ...
    def select_best_distribution(
        self, data: np.ndarray, candidate_distributions: List[str], method: str = "mle"
        ...
        for dist in candidate_distributions:
            try:
                ...
                if self.cv_enabled and "cv_likelihood" in self.selection_criteria:
                    cv_result = self.cross_validator.validate_distribution(
                        data, dist, method
                    )
        ...
        for criterion in self.selection_criteria:
            scores = self._calculate_ranking_scores(
                fitted_models, cv_results, criterion
            )
        ...
        sorted_models = sorted(
            fitted_models, key=lambda m: ranking_scores[m.distribution_type]
        )
        ...
        if criterion == "aic":
            ...
            for model in fitted_models:
                if model.distribution_type in cv_results:
                    ...
                    scores[model.distribution_type] = -cv_results[
...
def select_best_distribution(
    data: np.ndarray, candidate_distributions: List[str] = None, method: str = "mle"
) -> ModelComparisonResult:
...
def validate_model_performance(
    data: np.ndarray, distribution: str, method: str = "mle", n_splits: int = 5
) -> CrossValidationResult:
