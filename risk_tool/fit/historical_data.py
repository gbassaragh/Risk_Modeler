...
class DataQualityChecker:
    ...
    def assess_quality(self, data: Union[np.ndarray, pd.Series]) -> DataQualityReport:
        ...
        negative_pct = (n_negative / len(clean_values)) * 100 if len(clean_values) > 0 else 0
        outlier_pct = (n_outliers / len(clean_values)) * 100 if len(clean_values) > 0 else 0
    ...
    def _detect_outliers(self, data: np.ndarray) -> OutlierDetectionResult:
        ...
        detector = OutlierDetector(method=self.outlier_method, threshold=self.outlier_threshold)
    ...
    def _calculate_quality_score(self, 
                                ...
                                negative_pct: float,
                                outlier_pct: float,
                                n_valid: int) -> Tuple[float, List[str]]:
        ...
        if n_valid < 30:
            score -= 20
            recommendations.append("Increase sample size (need ≥30 for reliable fitting)")
        elif n_valid < 50:
            score -= 10
            recommendations.append("Consider collecting more data for better reliability")
        ...
        if zero_pct > 30:
            score -= 20
            recommendations.append("High rate of zero values - check data recording process")
        elif zero_pct > 15:
            score -= 10
            recommendations.append("Consider separate analysis for zero vs. non-zero values")
        
        # Negative values (usually problematic for cost data)
        if negative_pct > 10:
            score -= 30
            recommendations.append("Investigate negative values - may indicate data errors")
        elif negative_pct > 0:
            score -= 10
        ...
        if outlier_pct > 20:
            score -= 15
            recommendations.append("High outlier rate - consider robust fitting methods")
        elif outlier_pct > 10:
            score -= 10
            recommendations.append("Review outliers - may indicate special conditions or errors")
...
class OutlierDetector:
    ...
    SUPPORTED_METHODS = ['iqr', 'zscore', 'modified_zscore', 'isolation_forest', 'dbscan']
    ...
    def _detect_isolation_forest(self, data: np.ndarray) -> OutlierDetectionResult:
        ...
        try:
            ...
            warnings.warn("sklearn not available for Isolation Forest. Using IQR method.")
            return self._detect_iqr(data)
...
class SeasonalityAnalyzer:
    ...
    def analyze(self, 
                ...
                period: Optional[int] = None) -> SeasonalityResult:
        ...
        if period and period > 1:
            try:
                ...
                trend_component, seasonal_component, residual_component = self._decompose_series(
                    values, period
                )
                ...
                seasonal_factors = self._calculate_seasonal_factors(seasonal_component, period)
            
            except Exception as e:
                warnings.warn(f"Seasonal decomposition failed: {e}")
        
        # Determine if seasonality is significant
        has_seasonality = (seasonal_strength > 0.1 and 
    ...
    def _detect_seasonal_period(self, 
                               values: np.ndarray,
                               dates: Optional[np.ndarray] = None) -> Optional[int]:
...
class HistoricalDataProcessor:
    ...
    def process(self, 
        ...
        # Outlier detection
        clean_values = values.dropna() if hasattr(values, 'dropna') else values[~np.isnan(values)]
        ...
        if remove_outliers and outlier_result.n_outliers > 0:
            if hasattr(processed_values, 'iloc'):
                # pandas Series
                valid_indices = ~processed_values.index.isin(outlier_result.outlier_indices)
                processed_values = processed_values[valid_indices]
            else:
                # numpy array
                processed_values = np.delete(processed_values, outlier_result.outlier_indices)
    ...
    def _generate_processing_notes(self, 
                                  ...
                                  removed_outliers: bool) -> List[str]:
        ...
        if outlier_result.outlier_pct > 10:
            if removed_outliers:
                notes.append(f"Removed {outlier_result.n_outliers} outliers ({outlier_result.outlier_pct:.1f}%)")
            else:
                notes.append(f"Detected {outlier_result.n_outliers} outliers ({outlier_result.outlier_pct:.1f}%) - not removed")
...
def process_cost_data(data: Union[pd.DataFrame, pd.Series, np.ndarray],
                     value_column: Optional[str] = None,
                     date_column: Optional[str] = None,
...
def detect_outliers(data: np.ndarray, 
                   method: str = 'iqr',
                   threshold: float = 1.5) -> OutlierDetectionResult:
    """Detect outliers in data.
...
def analyze_seasonality(data: Union[pd.Series, np.ndarray],
                       dates: Optional[np.ndarray] = None) -> SeasonalityResult:
